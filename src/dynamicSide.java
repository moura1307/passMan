...
public class dynamicSide {
    ...
    public static void resizePasswordPage(JPanel passwordsPage, JPanel newPasswordPanel, ImageIcon newPasswordIcon, JButton addPanelButton) {
        ...
        if(newPasswordPanel.getWidth() <= 1250) {
            imageWidth = (int) (newPasswordPanel.getWidth() * 0.25);
        } else if(newPasswordPanel.getWidth()<= 1550) {
            imageWidth = (int) (newPasswordPanel.getWidth() * 0.2);
        } else {
    ...
    public static void resizeSiteImage(JPanel roundedPanel, ImageIcon imageHolderImage, JButton siteButton) {
        int imageHeight = roundedPanel.getHeight() - 150;
        int imageWidth = imageHeight;

        Image resizedPassword = imageHolderImage.getImage().getScaledInstance(imageWidth, imageHeight, Image.SCALE_SMOOTH);
        siteButton.setIcon(new ImageIcon(resizedPassword));
    }
}